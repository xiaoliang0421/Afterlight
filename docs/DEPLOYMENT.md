...
4. Apply migrations to the explicit target environment. Never seed a remote database with development fixtures. Parenthesize `CASE ... END` inside triggers: the remote D1 statement splitter can otherwise reject valid SQLite with `incomplete input` ([Cloudflare issue #4727](https://github.com/cloudflare/workers-sdk/issues/4727)). Local migration tests do not replace checking the remote migration result.
