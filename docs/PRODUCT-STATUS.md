...
- Draft legal pages, exact acceptance records, authenticated account-record downloads and account-deletion requests. Export includes paginated personal records, cancellation and a complete-file-only download; credentials and other users’ private records are excluded. Policies are not final; deletion execution is not implemented.
...
| High | Account deletion execution | Account-record export is implemented and tested. Remaining work: review unfinished tasks/orders, user-visible deletion outcomes, appropriate attribution anonymization, session removal and an audit trail. Final retention/operator policy must be settled before destructive execution is enabled. |
...
1. Public name is TaleRelay. Its .com registration is unresolved: the owner reports a bank authorization hold and a support estimate of about 30 minutes for registry provisioning. Do not repeat checkout or claim ownership until registration is confirmed. The owner supplied talerelay@proton.me for TaleRelay Support. Operator identity-disclosure wording and the remaining policy provisions still need finalization.
2. Configure a dedicated Google OAuth web client. Local callback testing can proceed before custom-domain registration; the hosted staging callback is also usable once its network path works. Add the final domain callback when registration completes. Test first sign-in, nickname setup and account recovery.
