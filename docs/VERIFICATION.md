...
9. Worker/Workflow deployment beyond the two private R2 buckets and two initialized D1 databases, a successful remote GitHub CI run, actual deployed-domain smoke tests and production release approval.
...
Staging static preflight now passes after resource IDs and a temporary workers.dev origin were configured. Worker dry-run: 2668.09 KiB, gzip 466.29 KiB. Production remains blocked on its origin, policies, protection and acceptance gates. A static pass does not establish a deployed site; local CLI authentication and remote deployment remain outstanding.
