...
## Current controlled-staging code validation

The latest code passes 71 application/database/runtime tests, TypeScript and build, plus 19 isolated API subtests (20 runner tests) and eight credential-helper tests. The native Workers regression reproduces the unsupported redirect option before the fix; the text-video regression reproduces omitted fixed setting/style before the fix. [The current summary](STAGING-VALIDATION.md) separates technical acceptance from the still-pending accepted clip and production release. Actual billing, account identifiers, provider request references and authorization records are retained privately.
